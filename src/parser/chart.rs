...
fn extract_chart_block(source: &str) -> Option<String> {
    ...
    for line in source.lines() {
        let t = line.trim();
        if inside { if t.starts_with("@endchart") { break; } lines.push(line); }
        else if t.starts_with("@startchart") { inside = true; }
    }
    ...
    for (n, line) in block.lines().enumerate() {
        ...
        if let Some(r) = t.strip_prefix("h-axis") {
            ...
            if let Some(labels) = parse_quoted_list(r) {
                if labels.len() == 1 && !r.contains(',') { xt = Some(labels[0].clone()); }
                else { xl = labels; }
            }
        } else if let Some(r) = t.strip_prefix("v-axis") {
            let r = r.trim();
            if let Some(p) = r.find('"') { if let Some(e) = r[p+1..].find('"') { yt = Some(r[p+1..p+1+e].to_string()); } }
        } else if let Some(r) = t.strip_prefix("bar") {
            if let Some((l, v)) = parse_series_line(r.trim()) {
                debug!("line {}: bar '{}' {} vals", n+1, l, v.len());
                sr.push(ChartSeries { label: l, values: v, series_type: ChartSeriesType::Bar });
            }
        } else { trace!("line {}: skip '{}'", n+1, t); }
    }
    Ok(ChartDiagram { x_labels: xl, series: sr, x_title: xt, y_title: yt })
}
fn parse_quoted_list(input: &str) -> Option<Vec<String>> {
    ...
    loop {
        ...
        if rest.starts_with('"') {
            if let Some(e) = rest[1..].find('"') { r.push(rest[1..1+e].to_string()); rest = rest[2+e..].trim_start_matches(',').trim_start(); }
            else { break; }
        } else { break; }
    }
...
fn parse_series_line(input: &str) -> Option<(String, Vec<f64>)> {
    ...
    let vals: Vec<f64> = rest.split(',').filter_map(|s| s.trim().parse().ok()).collect();
    if vals.is_empty() { None } else { Some((label, vals)) }
}
#[cfg(test)]
mod tests {
    use super::*;
