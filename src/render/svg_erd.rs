...
fn render_entity(sg: &mut SvgGraphic, node: &ErdNodeLayout, bg: &str, border: &str, font_color: &str) {
    ...
    if node.is_weak {
        ...
        let inset = 3.0;
        sg.set_fill_color(bg); sg.set_stroke_color(Some(border)); sg.set_stroke_width(0.5, None);
        sg.svg_rectangle(x + inset, y + inset, w - 2.0 * inset, h - 2.0 * inset, 0.0, 0.0, 0.0);
    } else {
        sg.set_fill_color(bg); sg.set_stroke_color(Some(border)); sg.set_stroke_width(0.5, None);
        sg.svg_rectangle(x, y, w, h, 0.0, 0.0, 0.0);
    }
...
fn render_attr_parent_lines(sg: &mut SvgGraphic, attrs: &[ErdAttrLayout], entities: &[ErdNodeLayout], relationships: &[ErdNodeLayout]) {
    for attr in attrs {
        if let Some((px, py)) = entities.iter().chain(relationships.iter()).find(|n| n.id == attr.parent).map(|n| (n.x + n.width / 2.0, n.y + n.height / 2.0)) {
...
fn render_note(sg: &mut SvgGraphic, note: &ErdNoteLayout) {
    if let Some((x1, y1, x2, y2)) = note.connector {
        sg.set_stroke_color(Some(NOTE_BORDER)); sg.set_stroke_width(1.0, Some((5.0, 3.0)));
        sg.svg_line(x1, y1, x2, y2, 0.0);
    }
    let (x, y, w, h) = (note.x, note.y, note.width, note.height); let fold = NOTE_FOLD;
...
mod tests {
    ...
    fn empty_diagram() -> ErdDiagram { ErdDiagram { entities: vec![], relationships: vec![], links: vec![], isas: vec![], direction: crate::model::erd::ErdDirection::TopToBottom, notes: vec![] } }
    fn empty_layout() -> ErdLayout { ErdLayout { entity_nodes: vec![], relationship_nodes: vec![], attribute_nodes: vec![], edges: vec![], isa_layouts: vec![], notes: vec![], width: 400.0, height: 300.0 } }
    fn make_entity_node(id: &str, x: f64, y: f64, w: f64, h: f64) -> ErdNodeLayout { ErdNodeLayout { id: id.into(), label: id.into(), x, y, width: w, height: h, is_weak: false, is_identifying: false } }
    fn make_attr(id: &str, parent: &str, x: f64, y: f64) -> ErdAttrLayout { ErdAttrLayout { id: id.into(), label: id.into(), parent: parent.into(), x, y, rx: 40.0, ry: 16.0, is_key: false, is_derived: false, is_multi: false, has_type: false, type_label: None, children: vec![] } }

    #[test] fn test_empty_diagram() { let svg = render_erd(&empty_diagram(), &empty_layout(), &SkinParams::default()).unwrap(); assert!(svg.contains("<svg")); assert!(svg.contains("</svg>")); assert!(svg.contains("xmlns=\"http://www.w3.org/2000/svg\"")); }
    #[test] fn test_entity_rect() { let mut l = empty_layout(); l.entity_nodes.push(make_entity_node("MOVIE", 50.0, 50.0, 100.0, 36.0)); let svg = render_erd(&empty_diagram(), &l, &SkinParams::default()).unwrap(); assert!(svg.contains("<rect")); assert!(svg.contains("MOVIE")); assert!(svg.contains(r#"lengthAdjust="spacing""#)); }
    #[test] fn test_weak_entity_double_border() { let mut l = empty_layout(); l.entity_nodes.push(ErdNodeLayout { is_weak: true, ..make_entity_node("CHILD", 50.0, 50.0, 100.0, 36.0) }); let svg = render_erd(&empty_diagram(), &l, &SkinParams::default()).unwrap(); assert_eq!(svg.matches("<rect").count(), 2); }
    #[test] fn test_relationship_diamond() { let mut l = empty_layout(); l.relationship_nodes.push(ErdNodeLayout { id: "RENTED_TO".into(), label: "RENTED_TO".into(), x: 50.0, y: 50.0, width: 100.0, height: 40.0, is_weak: false, is_identifying: false }); let svg = render_erd(&empty_diagram(), &l, &SkinParams::default()).unwrap(); assert!(svg.contains("<polygon")); assert!(svg.contains("RENTED_TO")); }
    #[test] fn test_identifying_relationship() { let mut l = empty_layout(); l.relationship_nodes.push(ErdNodeLayout { id: "PARENT_OF".into(), label: "PARENT_OF".into(), x: 50.0, y: 50.0, width: 120.0, height: 40.0, is_weak: false, is_identifying: true }); let svg = render_erd(&empty_diagram(), &l, &SkinParams::default()).unwrap(); assert_eq!(svg.matches("<polygon").count(), 2); }
    #[test] fn test_attribute_ellipse() { let mut l = empty_layout(); l.entity_nodes.push(make_entity_node("E", 100.0, 100.0, 80.0, 36.0)); l.attribute_nodes.push(make_attr("Code", "E", 100.0, 40.0)); let svg = render_erd(&empty_diagram(), &l, &SkinParams::default()).unwrap(); assert!(svg.contains("<ellipse")); assert!(svg.contains("Code")); }
