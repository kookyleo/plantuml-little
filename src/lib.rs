...
fn render_expanded(expanded: &str) -> Result<String> {
    ...
    let result = render_cleaned(&cleaned);
    render::svg_richtext::clear_sprites();
    result
}

fn render_cleaned(source: &str) -> Result<String> {
    ...
    let svg = render::svg::render(&diagram, &diagram_layout, &skin, &meta)?;
    Ok(svg)
}
