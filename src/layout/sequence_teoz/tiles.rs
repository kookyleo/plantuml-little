...
impl Tile for TileKind {
	fn preferred_height(&self) -> f64 {
		match self {
			TileKind::Communication { preferred_height, .. } => *preferred_height,
			TileKind::CommunicationSelf { preferred_height, .. } => *preferred_height,
			TileKind::LifeEvent { preferred_height, .. } => *preferred_height,
			...
			TileKind::Delay { preferred_height, .. } => *preferred_height,
...
impl TileKind {
	...
	pub fn min_x_adjusted(&self, rl: &mut RealLine) -> RealId {
		match self {
			...
			TileKind::Grouping { min_x, .. } => {
				rl.add_fixed(*min_x, -GROUPING_EXTERNAL_MARGINX1)
			}
			TileKind::Delay {
				middle, half_width, ..
	...
	pub fn max_x_adjusted(&self, rl: &mut RealLine) -> RealId {
		match self {
			...
			TileKind::Grouping { max_x, .. } => {
				rl.add_fixed(*max_x, GROUPING_EXTERNAL_MARGINX2)
			}
			TileKind::Delay {
				middle, half_width, ..
			} => rl.add_fixed(*middle, *half_width),
...
mod tests {
	...
	fn communication_preferred_height_basic() {
		...
		let tile = TileKind::communication(
			0, 1, c1, c2, &text,
			0.0, 0.0,
	...
	fn communication_preferred_height_with_inclination() {
		...
		let expected =
			18.0 + rose::ARROW_DELTA_Y + 2.0 * rose::ARROW_PADDING_Y + 5.0 + 3.0;
		assert!(
			(tile.preferred_height() - expected).abs() < f64::EPSILON,
			"got {}, expected {}",
			tile.preferred_height(),
	...
	fn communication_self_preferred_height() {
		...
		let tile = TileKind::communication_self(
			0, c, c2, &text,
			false, None, None, 0,
		);
		...
		let expected =
			text_h + rose::ARROW_DELTA_Y + rose::SELF_ARROW_ONLY_HEIGHT + 2.0 * rose::ARROW_PADDING_Y;
		assert!(
			(tile.preferred_height() - expected).abs() < f64::EPSILON,
			"got {}, expected {}",
			tile.preferred_height(),
	...
	fn callback_y_sets_position() {
		...
		assert!(
			(tile.get_y().unwrap() - 55.0).abs() < f64::EPSILON,
		);
	}
}
