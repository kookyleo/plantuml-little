...
mod arrow_direction {
	...
	fn reverse_left_to_right_gives_right_to_left() {
		assert_eq!(ArrowDirection::LeftToRight.reverse(), ArrowDirection::RightToLeft);
	}

	#[test]
	fn reverse_right_to_left_gives_left_to_right() {
		assert_eq!(ArrowDirection::RightToLeft.reverse(), ArrowDirection::LeftToRight);
	}
}
...
mod arrow_head {
	...
	fn values_four_variants_in_order() {
		...
		let all = [ArrowHead::Normal, ArrowHead::CrossX, ArrowHead::Async, ArrowHead::None];
		assert_eq!(all.len(), 4);
		assert_ne!(all[0], all[1]);
		assert_ne!(all[0], all[2]);
		assert_ne!(all[0], all[3]);
...
mod arrow_dressing {
	...
	fn get_head_after_with_head_async() {
		assert_eq!(ArrowDressing::none().with_head(ArrowHead::Async).head, ArrowHead::Async);
	}
...
mod arrow_configuration {
	...
	fn get_decoration1_default_is_none() {
		assert_eq!(ArrowConfiguration::with_direction_normal().decoration1(), ArrowDecoration::None);
	}

	#[test]
	fn get_decoration2_default_is_none() {
		assert_eq!(ArrowConfiguration::with_direction_normal().decoration2(), ArrowDecoration::None);
	}
	...
	fn get_color_default_is_none() {
		...
		assert!(ArrowConfiguration::with_direction_normal().color().is_none());
	}

	#[test]
	fn get_part_default_is_full() {
		assert_eq!(ArrowConfiguration::with_direction_normal().part(), ArrowPart::Full);
	}
...
mod component_type {
	...
	fn value_of_participant_head() {
		...
		assert_eq!(format!("{:?}", ComponentType::ParticipantHead), "ParticipantHead");
	}
...
mod component_style {
	...
	fn values_three_variants_in_order() {
		...
		let all = [ComponentStyle::Uml1, ComponentStyle::Uml2, ComponentStyle::Rectangle];
		assert_eq!(all.len(), 3);
		assert_ne!(all[0], all[1]);
		assert_ne!(all[0], all[2]);
		assert_ne!(all[1], all[2]);
...
mod actor_style {
	...
	fn value_of_stickman_business() {
		...
		assert_eq!(format!("{:?}", ActorStyle::StickmanBusiness), "StickmanBusiness");
	}
...
mod alignment_param {
	...
	fn value_of_arrow_message_alignment() {
		...
		assert_ne!(AlignmentParam::ArrowMessageAlignment, AlignmentParam::SequenceMessageAlignment);
		assert_eq!(
	...
	fn value_of_sequence_message_alignment() {
		assert_ne!(AlignmentParam::SequenceMessageAlignment, AlignmentParam::ArrowMessageAlignment);
		assert_eq!(
...
mod color_param {
	...
	fn value_of_class_background() {
		...
		assert_eq!(format!("{:?}", ColorParam::ClassBackground), "ClassBackground");
	}
	...
	fn value_of_note_background() {
		...
		assert_eq!(format!("{:?}", ColorParam::NoteBackground), "NoteBackground");
	}
	...
	fn value_of_activity_background() {
		...
		assert_eq!(format!("{:?}", ColorParam::ActivityBackground), "ActivityBackground");
	}
