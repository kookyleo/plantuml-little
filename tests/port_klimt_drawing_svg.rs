...
mod svg_graphic_tests {
	...
	fn body_not_empty_after_rectangle() {
		...
		assert!(!svg.body().is_empty(), "body should not be empty after drawing a rectangle");
	}
	...
	fn ellipse_with_fill_and_stroke() {
		...
		assert!(body.contains("fill=\"#FEFECE\""), "fill color should appear");
		assert!(body.contains("stroke:#A80036"), "stroke color should appear in style");
	}
	...
	fn arc_ellipse_produces_path() {
		...
		let body = svg.body();
		assert!(!body.is_empty(), "body should have content after arc ellipse");
		assert!(body.contains("<path"), "arc ellipse produces a <path element");
	}
	...
	fn gradient_different_policy_different_id() {
		...
		assert_ne!(id_pipe, id_dash, "different policy should produce different ids");
	}
	...
	fn gradient_defs_contain_linear_gradient() {
		...
		let defs = svg.defs();
		assert!(defs.contains("<linearGradient"), "defs should contain linearGradient");
	...
	fn stroke_with_dasharray() {
		...
		svg.svg_rectangle(0.0, 0.0, 100.0, 50.0, 0.0, 0.0, 0.0);
		let body = svg.body();
		assert!(body.contains("stroke-dasharray:5,3;"), "dasharray should appear in style");
	}
	...
	fn rectangle_with_rounded_corners() {
		...
		let body = svg.body();
		assert!(body.contains("rx=\"5\""), "rx attribute for rounded corners");
		assert!(body.contains("ry=\"5\""), "ry attribute for rounded corners");
	}
	...
	fn text_with_bold_and_italic() {
		...
		svg.svg_text(
			"styled", 0.0, 0.0,
			Some("serif"), 16.0,
			Some("bold"), Some("italic"), None,
			40.0, LengthAdjust::Spacing,
			None, 0, None,
		);
		let body = svg.body();
	...
	fn text_with_underline() {
		...
		svg.svg_text(
			"underlined", 0.0, 0.0,
			Some("sans-serif"), 14.0,
			None, None, Some("underline"),
			60.0, LengthAdjust::Spacing,
			None, 0, None,
		);
		let body = svg.body();
	...
	fn text_monospaced_becomes_monospace() {
		...
		svg.svg_text(
			"code", 0.0, 0.0,
			Some("monospaced"), 13.0,
			None, None, None,
			30.0, LengthAdjust::Spacing,
			None, 0, None,
		);
		let body = svg.body();
	...
	fn path_with_arc_to() {
		...
		svg.svg_path(0.0, 0.0, &path, 0.0);
		let body = svg.body();
		assert!(body.contains("A25,25"), "path data should contain Arc command");
	}
	...
	fn hidden_suppresses_rectangle() {
		...
		svg.set_stroke_width(1.0, None);
		svg.svg_rectangle(0.0, 0.0, 100.0, 50.0, 0.0, 0.0, 0.0);
		assert!(svg.body().is_empty(), "hidden rectangle should not appear in body");
	}
	...
	fn hidden_suppresses_ellipse() {
		...
		assert!(svg.body().is_empty(), "hidden ellipse should not appear in body");
	}
	...
	fn hidden_suppresses_line() {
		...
		assert!(svg.body().is_empty(), "hidden line should not appear in body");
	}
	...
	fn hidden_suppresses_polygon() {
		...
		assert!(svg.body().is_empty(), "hidden polygon should not appear in body");
	}
	...
	fn hidden_suppresses_path() {
		...
		svg.svg_path(0.0, 0.0, &path, 0.0);
		assert!(svg.body().is_empty(), "hidden path should not appear in body");
	}
	...
	fn hidden_suppresses_text() {
		...
		svg.svg_text(
			...
			Some("sans-serif"), 14.0,
			None, None, None,
			50.0, LengthAdjust::Spacing,
			None, 0, None,
		);
		assert!(!svg.body().contains("<text"), "hidden text should not produce <text");
	}
	...
	fn unhide_resumes_output() {
		...
		svg.svg_rectangle(0.0, 0.0, 100.0, 50.0, 0.0, 0.0, 0.0);
		assert!(svg.body().contains("<rect"), "after unhiding, rect should appear");
	}
	...
	fn link_produces_anchor() {
		...
		assert!(body.contains("href=\"http://example.com\""), "href attribute");
	...
	fn link_without_tooltip_uses_url_as_title() {
		...
		svg.close_link();
		let body = svg.body();
		assert!(body.contains("title=\"http://example.com\""),
	...
	fn link_xlink_attributes() {
		...
		svg.close_link();
		let body = svg.body();
		assert!(body.contains("xlink:href=\"http://test.org\""), "xlink:href");
	...
	fn to_svg_produces_svg_document() {
		...
		assert!(doc.contains("xmlns=\"http://www.w3.org/2000/svg\""), "SVG namespace");
	...
	fn to_svg_contains_body_content() {
		...
		assert!(doc.contains("<rect"), "full SVG document should contain the rect");
	}
	...
	fn to_svg_with_background_color() {
		...
		assert!(doc.contains("background:#FFFF00"), "background color in style");
	}
	...
	fn to_svg_transparent_bg_omitted() {
		...
		assert!(!doc.contains("background:"), "transparent bg should not appear");
	}
	...
	fn to_svg_empty_diagram_type_omitted() {
		let svg = make();
		let doc = svg.to_svg(None, "");
		assert!(!doc.contains("data-diagram-type"), "empty type should be omitted");
	}
	...
	fn to_svg_contains_plantuml_pi() {
		let svg = make();
		let doc = svg.to_svg(None, "");
		assert!(doc.contains("<?plantuml"), "should contain PlantUML processing instruction");
	}
	...
	fn shadow_creates_filter_in_defs() {
		...
		assert!(defs.contains("feGaussianBlur"), "Gaussian blur in shadow filter");
		let body = svg.body();
		assert!(body.contains("filter=\"url(#"), "rect should reference shadow filter");
	}
	...
	fn polyline_basic() {
		...
		let body = svg.body();
		assert!(body.contains("<polyline"), "should contain <polyline element");
		assert!(body.contains("points=\""), "should have points attribute");
	}
	...
	fn scale_accessor() {
		...
		assert!((svg.scale() - 2.0).abs() < f64::EPSILON, "scale() returns construction scale");
	}
	...
	fn transparent_fill_becomes_none() {
		...
		svg.svg_rectangle(0.0, 0.0, 50.0, 30.0, 0.0, 0.0, 0.0);
		let body = svg.body();
		assert!(body.contains("fill=\"none\""), "#00000000 should become fill=none");
	}
	...
	fn stroke_width_zero_omits_style() {
		...
		svg.svg_rectangle(0.0, 0.0, 50.0, 30.0, 0.0, 0.0, 0.0);
		let body = svg.body();
		assert!(!body.contains("style="), "stroke-width 0 should not emit style attribute");
	}
	...
	fn text_back_color_creates_filter() {
		...
		let body = svg.body();
		assert!(body.contains("filter=\"url(#"), "text should reference back-color filter");
		let defs = svg.defs();
		assert!(defs.contains("flood-color=\"#FFFF00\""), "filter defs should have flood-color");
	}
	...
	fn extents_grow_with_shapes() {
		...
		assert!(svg.max_x() > initial_x, "max_x should grow after large rect");
		assert!(svg.max_y() > initial_y, "max_y should grow after large rect");
	}
}
...
mod ugraphic_svg_tests {
	...
	fn ugraphic_draw_rect_produces_rect() {
		...
		assert!(body.contains("<rect"), "draw_rect should produce <rect in SVG body");
	}
	...
	fn ugraphic_draw_line_produces_line() {
		...
		let body = ug.svg().body();
		assert!(body.contains("<line"), "draw_line should produce <line in SVG body");
	}
	...
	fn ugraphic_draw_ellipse_produces_ellipse() {
		...
		let body = ug.svg().body();
		assert!(body.contains("<ellipse"), "draw_ellipse should produce <ellipse");
	}
	...
	fn ugraphic_draw_polygon_produces_polygon() {
		...
		let body = ug.svg().body();
		assert!(body.contains("<polygon"), "draw_polygon should produce <polygon");
	}
	...
	fn ugraphic_into_svg_returns_graphic() {
		...
		let svg = ug.into_svg();
		assert!(svg.body().contains("<rect"), "into_svg should preserve drawn content");
	}
