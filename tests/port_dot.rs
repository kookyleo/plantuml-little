...
use plantuml_little::dot::{
	DotSplines, ExeState, GraphvizVersion, ProcessState, Point2D, Rect2D,
...
mod process_state_tests {
	...
	fn to_string_exception_includes_cause_message() {
		...
		let s = format!("{state}");
		assert!(s.starts_with("EXCEPTION"), "should start with EXCEPTION, got: {s}");
		assert!(s.contains("dot crashed"), "should contain cause message, got: {s}");
	}
...
mod exe_state_tests {
	...
	fn text_message_does_not_exist() {
		...
		assert_eq!(ExeState::DoesNotExist.text_message(), "Dot executable does not exist");
	}
	...
	fn text_message_not_a_file() {
		...
		assert_eq!(ExeState::NotAFile.text_message(), "Dot executable is not a valid file");
	}

	#[test]
	fn text_message_cannot_be_read() {
		// Java: assertEquals("Dot executable cannot be read", ExeState.CANNOT_BE_READ.getTextMessage())
		assert_eq!(ExeState::CannotBeRead.text_message(), "Dot executable cannot be read");
	}
	...
	fn text_message_with_path_ok_contains_path_and_ok() {
		...
		assert!(msg.contains("/usr/bin/dot"), "should contain path, got: {msg}");
	...
	fn text_message_with_path_does_not_exist_contains_path() {
		...
		assert!(msg.contains("does not exist"), "should describe absence, got: {msg}");
	}
...
mod dot_splines_tests {
	...
	fn rust_has_at_least_three_of_the_java_variants() {
		...
		let all = [DotSplines::Splines, DotSplines::Polyline, DotSplines::Ortho, DotSplines::Curved];
		assert_eq!(all.len(), 4);
	...
	fn polyline_parses_from_name() {
		...
		assert_eq!(DotSplines::from_str_opt("polyline"), Some(DotSplines::Polyline));
	}
	...
	fn splines_parses_from_name() {
		...
		assert_eq!(DotSplines::from_str_opt("splines"), Some(DotSplines::Splines));
	}
...
mod graphviz_version_finder_tests {
	...
	fn version_with_missing_exe_uses_default_flags() {
		...
		let new_v = GraphvizVersion { major: 2, minor: 44, patch: 0 };
		assert!(!new_v.use_shield_for_quantifier(), "v2.44 should not need quantifier shield");
	}
...
mod graphviz_linux_tests {
	use plantuml_little::dot::graphviz::{GraphvizNative, Graphviz};
	...
	fn graphviz244_on_windows_is_false() {
		...
		use std::path::Path;
		let state = ExeState::check_file(Some(Path::new("/nonexistent/dot_that_does_not_exist_xyz")));
		assert_eq!(state, ExeState::DoesNotExist, "non-existent exe path must be DoesNotExist");
	...
	fn get_dot_string_returns_string_passed_at_construction() {
		...
		let is_valid = matches!(
			state,
			ExeState::Ok | ExeState::DoesNotExist | ExeState::NullUndefined
				| ExeState::IsADirectory | ExeState::NotAFile | ExeState::CannotBeRead
		);
		assert!(is_valid, "exe_state must be a recognized ExeState variant, got {:?}", state);
	}
	...
	fn exe_state_is_inspectable() {
		...
		match state {
			...
			ExeState::NullUndefined => assert!(gv.dot_exe().is_none(), "NullUndefined must have no path"),
...
mod process_runner_tests {
	...
	fn run_cat_with_stdin_input_echoes_back() {
		...
		assert!(result.state.is_ok(), "cat should succeed, got: {}", result.state);
		let out = String::from_utf8_lossy(&result.stdout);
