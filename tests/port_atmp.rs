...
mod cuca_diagram_tests {
	...
	fn parse_class_diagram_entity_names() {
		...
		assert!(names.contains(&"A"), "should contain entity A, got {:?}", names);
		assert!(names.contains(&"B"), "should contain entity B, got {:?}", names);
	}
	...
	fn parse_class_diagram_no_groups() {
		...
		let cd = parse_class(class_diagram_source());
		assert!(cd.groups.is_empty(), "plain class diagram should have no groups");
	}
	...
	fn parse_minimal_diagram_single_entity() {
		...
		assert!(cd.links.is_empty(), "single class diagram should have no links");
	}
	...
	fn parse_interface_entity_kind() {
		...
		assert_eq!(cd.entities[0].kind, plantuml_little::model::EntityKind::Interface);
	}
	...
	fn parse_enum_entity_kind() {
		...
		let cd = parse_class(src);
		assert_eq!(cd.entities.len(), 1);
		assert_eq!(cd.entities[0].kind, plantuml_little::model::EntityKind::Enum);
	}
	...
	fn parse_abstract_class_entity_kind() {
		...
		let cd = parse_class(src);
		assert_eq!(cd.entities.len(), 1);
		assert_eq!(cd.entities[0].kind, plantuml_little::model::EntityKind::Abstract);
	}
	...
	fn convert_class_diagram_svg_contains_entity_names() {
		...
		assert!(svg.contains(">B<") || svg.contains(">B</"),
	...
	fn convert_class_with_members_produces_svg_containing_members() {
		...
		let svg = convert(src).expect("convert should succeed");
		assert!(svg.contains("<svg"), "output must contain <svg tag");
		assert!(svg.contains("doSomething"), "SVG should contain method name");
	}
	...
	fn parse_diagram_with_multiple_links() {
		...
		assert_eq!(cd.links.len(), 3, "three relationship lines should yield 3 links");
	}
	...
	fn parse_entity_with_stereotype() {
		...
		assert_eq!(cd.entities.len(), 1);
		assert!(
			cd.entities[0].stereotypes.iter().any(|s| s.0.contains("entity")),
	...
	fn parse_dashed_link_style() {
		...
		assert_eq!(cd.links[0].line_style, plantuml_little::model::LineStyle::Dashed);
	}
	...
	fn parse_inheritance_link_has_triangle_head() {
		...
		let cd = parse_class(src);
		assert_eq!(cd.links.len(), 1);
		assert_eq!(cd.links[0].right_head, plantuml_little::model::ArrowHead::Triangle);
	}
	...
	fn convert_svg_has_width_and_height() {
		...
		assert!(has_dimensions, "SVG should have width/viewBox dimension attributes");
	}
